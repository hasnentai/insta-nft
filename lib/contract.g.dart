...
class Contract extends _i1.GeneratedContract {
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> createNFT(_i1.EthereumAddress player, String tokenURI,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[3];
    assert(checkSignature(function, 'a5614033'));
  ...
  /// Returns a live stream of all NFTMinted events emitted by this contract.
  Stream<NFTMinted> nFTMintedEvents(
      {_i1.BlockNum? fromBlock, _i1.BlockNum? toBlock}) {
    final event = self.event('NFTMinted');
    final filter = _i1.FilterOptions.events(
        contract: self, event: event, fromBlock: fromBlock, toBlock: toBlock);
    return client.events(filter).map((_i1.FilterEvent result) {
      final decoded = event.decodeResults(result.topics!, result.data!);
      return NFTMinted(decoded);
    });
  }
...
class NFTMinted {
  NFTMinted(List<dynamic> response) : newItemId = (response[0] as BigInt);

  final BigInt newItemId;
}
